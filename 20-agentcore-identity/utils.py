...
def setup_cognito_user_pool(pool_name="MCPServerPool"):
    ...
    try:
        ...
        # Create App Client with OAuth authorization code flow and OpenID scopes
        # This ensures ID tokens are returned with 'aud' claim for AgentCore Runtime validation
        app_client_response = cognito_client.create_user_pool_client(
            UserPoolId=pool_id,
            ClientName="MCPServerPoolClient",
            GenerateSecret=False,
            ExplicitAuthFlows=["ALLOW_USER_PASSWORD_AUTH", "ALLOW_REFRESH_TOKEN_AUTH"],
            AllowedOAuthFlows=["code"],  # OAuth authorization code flow
            AllowedOAuthScopes=["openid", "email", "profile"],  # OpenID scopes for ID tokens
            AllowedOAuthFlowsUserPoolClient=True,
            CallbackURLs=["http://localhost:3000/callback"],  # Required for OAuth flows
